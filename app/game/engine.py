...
class CricketEngine:
    ...
    def play(self, match, bat, bowl, owner_id):
        ...
        # Owner powers belong to the owner's own account, not every teammate
        # represented by a team captain. Teammates must remain dismissible.
        if striker_before.uid == owner_id:
            runs, wicket = random.choice([4, 6]), False
            text = "👑 OWNER POWER  •  " + (
                "🔥 FOUR!" if runs == 4 else "💥 SIX!"
            )
        elif bowler_before.uid == owner_id:
            runs, wicket = 0, True
