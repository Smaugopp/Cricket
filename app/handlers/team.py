...
async def team_cmd(message: Message, teams, users):
    ...
    if action == "list":
        ...
        await message.answer(
            "👥 <b>GROUP TEAMS</b>\n"
            "━━━━━━━━━━━━━━━━━━\n\n" +
            ("\n".join(f"🏏 <b>{t['name']}</b>  •  {len(t.get('players', []))} players" for t in rows) or "No teams yet.\nCreate one with <code>/team create NAME</code>"),
            parse_mode="HTML"
        ); return
    ...
    if action == "xi":
        team = await teams.my_team(message.chat.id, message.from_user.id)
        # Allow captain to use set/clear; everyone can view.
        ...
        if team["captain"] != message.from_user.id:
            await message.answer("❌ Captain only."); return
        if sub == "clear":
            await teams.clear_xi(team)
            await message.answer("✅ Playing XI cleared."); return
        ids = parts[3:] if len(parts) > 3 and parts[2].lower() == "set" else parts[2:]
        if len(ids) != 11 or not all(x.isdigit() for x in ids):
            await message.answer("Usage: /team xi set ID1 ID2 ... ID11"); return
