...
async def teamplay(message: Message, matches, teams, users, admin, db):
    ...
    if len(roster) not in {4, 5}:
        await message.answer(
            ...
            "Captain: use /team players ID1 ID2 ID3 ID4 [ID5]."
        )
        return
...
async def _resolve_result(bot, message, match, users, db, matches, settings, bat, bowl):
    ...
    if result.wicket and match.mode == "team" and match.innings:
        replacement = match.innings.batter
        if replacement.uid != before_batter.uid and replacement.uid not in match.innings.dismissed:
            wicket_line += f"➡️ <b>{replacement.name} IN</b>\n"
