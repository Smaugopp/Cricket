...
async def _send_batting_prompt(bot, match):
    ...
    if not i:
        return
    caption = (
        "🏏 <b>CRICKET ARENA • LIVE</b>\n"
        "━━━━━━━━━━━━━━━━━━\n\n"
        "⚡ <b>YOUR BATTING TURN</b>\n\n"
        f"👤 <b>{i.batter.name}</b>\n"
        "🔢 Choose your shot  <b>1–6</b>\n\n"
        "🎯 The bowler has already committed the delivery.\n"
        "━━━━━━━━━━━━━━━━━━"
    )
    try:
...
async def _resolve_result(bot, message, match, users, db, matches, settings, bat, bowl):
    ...
    await bot.send_message(
        match.chat_id,
        "🏏 <b>CRICKET ARENA • BALL RESULT</b>\n"
        "━━━━━━━━━━━━━━━━━━\n\n"
        f"⚾ <b>{match.innings.over(match.balls_per_over)}</b>  •  {result.ball_type}\n\n"
        f"🏏 <b>{before_batter.name}</b>  vs  🎯 <b>{before_bowler.name}</b>\n"
        f"{result.text}\n"
        f"{wicket_line}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"📊 <b>{score_text(match)}</b>",
        parse_mode="HTML",
    )
