...
async def commands(message: Message):
    await message.answer(
        ...
        "/tournament /league\n\n"
        "👤 <b>CAREER</b>\n"
        "/profile /mystats /stats /history /achievements /daily /leaderboard\n\n"
        "ℹ️ <b>UTILITY</b>\n"
        "/id /rules /ping\n\n"
        "🛡 <b>STAFF</b>\n"
...
async def botstats_alias(message: Message, settings):
    if message.from_user.id != settings.owner_id:
        await message.answer("❌ Owner-only command.")
        return
    await message.answer(
        "📡 <b>OWNER BOT DASHBOARD</b>\n\n"
        "Use <code>/stats</code> for users, groups, matches, live matches and teams.",
        parse_mode="HTML",
    )
